...
local function buildSector(canvas, sector)
    ...
    if NS.DEAD[sector] then
        local dot = canvas:CreateTexture(nil, "ARTWORK")
        dot:SetSize(DOT, DOT)
        dot:SetPoint("CENTER", canvas, "CENTER", ox, oy)
        dot:SetTexture("Interface\\CharacterFrame\\TempPortraitAlphaMask")
        dot:SetVertexColor(0.35, 0.35, 0.35, 0.7)
        local tag = canvas:CreateFontString(nil, "OVERLAY", "GameFontDisableSmall")
    ...
    btn:SetScript("OnEnter", function(self)
        ...
        if not InCombatLockdown() then
            GameTooltip:AddLine("Right-click to put a different marker here", 0.6, 0.6, 0.6)
        end
        GameTooltip:Show()
    end)
...
local function pairsText()
    ...
    for _, sector in ipairs(NS.SECTORS) do
        local opp = NS.Opposite(sector)
        if not NS.DEAD[sector] and not shown[sector] then
            shown[sector], shown[opp] = true, true
            parts[#parts + 1] = ("%s%s"):format(NS.MarkIcon(NS.MarkAt(sector), 13), NS.MarkIcon(NS.MarkAt(opp), 13))
        end
    end
